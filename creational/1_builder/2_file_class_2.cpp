/*
	We faced the issue of having duplicate code in constructors.
	We solved that issue by delegating constructors.

	We still have constructors that accept a lot of parameters, this makes it
	difficult for the clients to create instances of such class.


	// Constructor for simple files
	File(const char* fileName, DWORD desiredAccess)

	Even though we have written a comment here that this constructor is used for simple files,
	the clients may not know that because the constructor does not communicate that it is used
	for simple files.
	Also we can not change the name of the constructor to state its purpose.


	So we have a situation where we have a complex class and the instances of such class will
	have different kinds of states and for different kinds of states the constructors accept
	a lot of parameters.
	This is an indication that the objects of this class should be constructed step-by-step
	rather than in one single shot.

	Thus we can use the Builder Design Pattern.
